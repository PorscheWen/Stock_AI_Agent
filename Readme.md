...
台股妖股自動偵測與買賣策略系統。**Multi-Agent 架構**，每日台股收盤後透過 GitHub Actions 自動掃描全市場（**TWSE 上市 + TPEX 上櫃**），識別妖股並輸出完整的進出場操作建議，推播至 LINE Bot。
...
│   ├── operation_advice_agent.py 🧭 整體操作建議（市場節奏/倉位）
...
ORCHESTRATOR（主控）
    │
    ├─ [Step 1] ScannerAgent ──────► 全市場掃描（TWSE 上市 + TPEX 上櫃）
    │           找出：漲停股 + 量比≥5x + 連板數
    │                         │
    ├─ [Step 2] 並行執行 ──────┤
...
| Agent | 職責 | 輸出 |
|-------|------|------|
| **ScannerAgent** | TWSE（上市）+ TPEX（上櫃）全市場掃描，找漲停 + 量比 ≥ 5x + 計算連板數 | 妖股候選清單 |
